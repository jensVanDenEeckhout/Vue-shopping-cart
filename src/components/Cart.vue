<template>
    ...
    <b-container>
      ...
      <b-row>
        <b-col v-for="product in products" :key="product.id">
          ...
          <b-button v-if="!product.card" @click="add(product)" href="#" variant="primary">Add to Shopping Cart</b-button>
      ...
      <b-col>
        <b-table bordered hover :items="cart" :fields="fields">
          ...
          <template slot="remove" slot-scope="data">
            <b-button @click="remove(data.item.id)" variant="danger" class="mr-2"> X
            </b-button>
          </template>
        ...
        <template slot="image" slot-scope="data">
          <b-img style="max-width: 5rem;" :src= "require(`@/assets/fruit${data.item.id}.jpg`)" fluid alt="Responsive image"></b-img>
  ...
  <b-row v-if="cart.length > 0">
    ...
    <b-col>
    <b-button @click="buy" variant="succes" block class="mr-2">
    Buy
    </b-button>    
  </b-col>
</b-row>
...
<script>
  export default{
    ...
    decrement(id){
      for (let index = 0; index < index.cart.length; index++){
        if (this.cart[index].id == id){
          this.cart[index].quantity--
        }
      }
